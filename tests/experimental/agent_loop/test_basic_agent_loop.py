...
"""
基础 Agent Loop（智能体循环）功能的实验性测试。

测试内容包括：
- 单轮对话（single_turn）：验证基本的序列生成、reward 计算、轮数检查
- 工具调用 Agent（tool_agent）：验证多轮工具调用、response_mask 正确性、工具响应屏蔽
- 带交互的工具 Agent（tool_agent_with_interaction）：验证交互配置与工具调用的集成
- 轨迹信息获取（get_trajectory_info）：验证异步轨迹信息方法的正确性

同时定义了测试用工具类 WeatherTool 和 WeatherToolWithData，模拟天气查询工具。
"""
