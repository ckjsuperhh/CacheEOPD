...
"""
独立 Rollout（推理展开）服务的实验性测试。

测试内容包括：
- 独立 Rollout 服务器（standalone_rollout）：测试单节点和多节点的独立推理展开
- 混合 Rollout 与专家并行（hybrid_rollout_with_ep）：测试 DP=2, TP=4, EP=8 的混合推理配置

验证 FSDP 模型与 Rollout 服务的集成，包括 sleep/wake_up 机制和异步 OpenAI 接口调用。
"""
