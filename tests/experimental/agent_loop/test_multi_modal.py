...
"""
多模态 Agent Loop（智能体循环）的实验性测试。

测试内容包括：
- 多模态工具 Agent（multimodal_tool_agent）：使用 Qwen VL 模型验证图像生成工具的调用与返回
- 多模态单轮 Agent（multimodal_single_turn_agent）：验证图像/视频输入的单轮对话处理
- 多模态部分单轮 Agent（multimodal_partial_single_turn_agent）：验证部分单轮场景（当前已跳过）

定义了 ImageGeneratorTool 测试工具类，用于生成简单彩色图像以测试多模态工具调用流程。
"""
