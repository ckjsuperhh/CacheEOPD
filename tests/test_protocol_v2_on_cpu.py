...
使用原始 TensorDict 替代 DataProto 的 v2 版本数据协议 CPU 单元测试。

测试基于 tensordict_utils (tu) 工具函数的数据操作，包括：
- TensorDict 的合并操作与 NonTensorData 冲突检测
- 构造、索引选择、图像数据处理、嵌套张量打包
- TensorDict 相等性判断、迭代器创建、重排序、分块/拼接
- 数据弹出（pop）、获取（get）、重复（repeat）、填充/反填充
- 序列化保存与加载、索引操作、选择操作
- 嵌套列表/字典/复杂结构的非张量数据赋值
- 自动检测嵌套结构并转换为 NonTensorStack
- 完整的 agent loop 场景模拟测试
"""
