...
中文说明:
    模型 checkpoint 合并与验证工具（legacy 版本）。

    文件作用：
        将分布式训练产生的 FSDP 或 Megatron 分片 checkpoint 合并为
        完整的 HuggingFace 格式模型，便于推理评测或继续训练。

    支持的后端：
        - FSDP: 自动检测 world_size，加载所有 rank 的分片，
          根据 DTensor 的 placement 信息（Shard/Replicate）拼接还原
        - Megatron: 处理 TP（张量并行）和 PP（流水线并行）的分片，
          支持 QKV 拆分、gate/up 拆分、MoE 专家权重合并
    ...
    主要类：
        - ModelMergerConfig: 合并操作的数据类配置
        - BaseModelMerger: 抽象基类，提供通用的 HF 模型保存/tokenizer 保存/LoRA 适配器提取
        - FSDPModelMerger: FSDP 后端合并实现
        - MegatronModelMerger: Megatron 后端合并实现

    在 EOPD 中的角色：
        训练完成后，FSDP 分片权重需通过本脚本（或新版 verl.model_merger）
        合并为标准 HuggingFace 格式，才能用于离线评测。
