...
#
# 文件作用：
#   一键安装 verl 框架运行所需的所有 Python 依赖包，包括：
#   推理框架（vLLM、SGLang）、基础库、FlashAttention、FlashInfer、
#   TransformerEngine 和 Megatron-LM（可选）。
#
# 环境变量：
#   USE_MEGATRON: 是否安装 Megatron-LM 和 TransformerEngine（默认 1 = 安装）
#   USE_SGLANG:   是否安装 SGLang 推理框架（默认 1 = 安装）
#   MAX_JOBS:     编译并行度（默认 32，用于 flash-attn 等需要编译的包）
#
# 执行流程：
#   1. 安装推理框架：SGLang 0.5.2 + vLLM 0.11.0
#   2. 安装基础依赖：transformers, accelerate, datasets, ray, wandb 等
#   3. 安装 FlashAttention 2.8.1 和 FlashInfer 0.3.1
#   4. （可选）安装 TransformerEngine v2.6 + Megatron-LM core_v0.13.1
#   5. 修复 OpenCV 兼容性问题
#   6. （可选）安装 cuDNN Python 包
#
# 注意：
#   - TransformerEngine 安装耗时较长，请耐心等待
#   - cuDNN 包需在 TransformerEngine 之后安装，避免被覆盖
#   - pyext 不兼容 Python 3.12，如需使用请安装 patched fork
# =============================================================================

# 是否安装 Megatron 后端（1=安装，0=跳过）
USE_MEGATRON=${USE_MEGATRON:-1}
# 是否安装 SGLang 推理框架（1=安装，0=跳过）
USE_SGLANG=${USE_SGLANG:-1}

# 设置编译并行度，加速需要编译的包（如 flash-attn）的安装
export MAX_JOBS=32
