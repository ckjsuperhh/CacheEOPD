...
中文说明:
    环境诊断脚本，用于检查当前运行环境的各项配置是否满足 verl 框架要求。

    检查项目:
        - Python 版本、编译器、架构
        - pip 版本及已安装的关键包（vllm, sglang, ray, torch）
        - verl 版本及 git commit hash
        - 操作系统 / 平台信息
        - CPU / GPU 硬件信息（GPU 型号、显存）
        - CUDA 版本（runtime + compiler）
        - 网络连接（PyPI / 国内镜像）
        - 环境变量（VERL_*, OMP_*, KMP_*）

    命令行参数:
        --python: 检查 Python 信息（默认开启）
        --pip: 检查 pip 信息（默认开启）
        --verl: 检查 verl 信息（默认开启）
        --system: 检查系统信息（默认开启）
        --os: 检查 OS 信息（默认开启）
        --environment: 检查环境变量（默认开启）
        --hardware: 检查硬件信息（默认关闭）
        --network: 检查网络连接（默认关闭）
        --region: 指定额外测试的区域镜像（如 "cn" 测试国内镜像）
        --timeout: 连接测试超时时间（秒），0 为禁用

    使用方式:
        python scripts/diagnose.py          # 检查默认项目
        python scripts/diagnose.py --hardware --network --region cn  # 加检硬件和网络
"""
