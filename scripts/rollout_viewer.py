...
"""
Rollout 数据可视化查看器（TUI 终端界面）。

文件作用：
    提供一个交互式终端 UI（基于 Textual 框架），用于查看和浏览
    RL 训练过程中保存的 rollout 数据（JSONL 格式）。
    支持按 step/sample 导航、关键词搜索、字段过滤、排序等功能。

命令行用法：
    python scripts/rollout_viewer.py run <rollout_data_dir>
    # 可选参数：--mask-str 指定需要脱敏的字符串模式

执行流程：
    1. 扫描指定目录下所有 *.jsonl 文件（按文件名数字排序，每个文件对应一个 step）
    2. 异步加载第一个文件的数据并启动 TUI
    3. 后台继续加载其余文件
...
依赖：
    - textual==0.52.1: TUI 框架
    - typer: 命令行解析
    - rich: 富文本渲染
    - aiofiles: 异步文件 I/O

数据格式要求：
    - rollout_data_dir/ 下包含以数字命名的 .jsonl 文件（如 0.jsonl, 1.jsonl ...）
    - 每行是一个 JSON 对象，字段包含 prompt, response, score 等
"""
