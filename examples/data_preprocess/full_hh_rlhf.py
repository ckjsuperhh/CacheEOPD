...
"""
Full-HH-RLHF 数据集预处理脚本（Anthropic HH-RLHF 完整版）。

文件作用：
    将 Anthropic HH-RLHF 数据集拆分为 SFT / RM / RL 三种用途的子集，
    分别保存为 parquet 格式。

命令行参数：
    --split: 指定生成哪种数据集（"sft" | "rm" | "rl"，必需）
    --local_dir: （已弃用）本地保存目录
    --local_save_dir: 保存路径，默认 ~/data/full_hh_rlhf
    --local_dataset_path: 本地原始数据集路径（离线模式）
    --hdfs_dir: HDFS 保存目录（可选）

数据来源：Dahoas/full-hh-rlhf

各 split 说明：
    - sft: 使用 train 全集，每条数据含 prompt + chosen 和 prompt + rejected（双倍数据）
    - rm: 训练集取 75%，测试集取 25%，每条含 prompt + chosen + rejected
    - rl: 使用 train 全集，仅保留 prompt（response 在 RL 过程中由模型生成）

输出格式 (parquet, 以 rl 为例)：
    - data_source: "Dahoas/full-hh-rlhf"
    - prompt: [{"role": "user", "content": prompt 文本}]
    - ability: "alignment"
    - reward_model: {"style": "model", "ground_truth": response（仅供参考，RL 时不用）}
"""
