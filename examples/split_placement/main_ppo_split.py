...
"""
Split Placement PPO 训练主入口。

文件作用：
    将 Actor 和 Critic 分别放置在不同的 GPU 组上运行（Split Placement），
    而非标准的共置方式。适用于 Actor 和 Critic 模型尺寸差异大、
    或需要更灵活资源分配的场景。

    注意：此文件独立于 ray_trainer，因为 ray_trainer 被其他 main 入口共用。

依赖：
    - hydra: 配置管理
    - ray: 分布式调度
    - split_monkey_patch: 修改 verl 内部调度逻辑
"""
