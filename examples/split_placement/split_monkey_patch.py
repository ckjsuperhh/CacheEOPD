...
"""
Split Placement Monkey Patch — 修改 verl 内部资源调度逻辑。

文件作用：
    通过 monkey patch 方式修改 verl 框架的资源分配逻辑，
    使 Actor 和 Critic 可以被分配到不同的 GPU 子集上运行。
    这是 Split Placement 策略的核心实现。

主要修改：
    - 资源调度器：将 Actor/Critic 的 GPU 资源解耦
    - 训练循环：支持异步调度 Actor 和 Critic 的前向/反向传播
"""
