<!-- 
中文摘要：
CollabLLM 是一个训练语言模型在多轮对话中高效协作的方法（ICML 2025）。
该 recipe 使用 verl 框架实现了 CollabLLM 的训练流程，包括：
1. 数据预处理（process_dataset.py）：将多轮对话数据集转换为 RL/SFT 格式
2. 监督微调（train_sft_collabllm.sh）：用高质量对话数据进行 SFT
3. 强化学习训练（train_rl_collabllm.sh）：使用 GRPO 算法进行 RL 训练，
   通过用户模拟器生成多轮对话，并使用 LLM-as-Judge 计算多任务奖励
4. 评估指标（metrics/）：准确性、BLEU分数、交互性、代码通过率、token数量等
-->
