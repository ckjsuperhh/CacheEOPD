...
# CollabLLM 强化学习训练脚本
#
# 用途：使用 GRPO 算法对 CollabLLM 进行强化学习训练。
# 启动的训练任务：通过 verl 框架的 main_ppo 入口启动 PPO/GRPO 训练。
#
# 关键参数含义：
# - algorithm.adv_estimator=grpo: 使用 GRPO 作为优势函数估计方法
# - reward_model.reward_manager=collabllm: 使用 CollabLLM 自定义奖励管理器
# - metric_weights: 多任务奖励权重（准确性=1, 交互性=1, token数量=-0.0001）
# - actor_rollout_ref.rollout.multi_turn: 多轮对话 rollout 配置
#   - max_user_turns/max_assistant_turns: 最大对话轮次
#   - num_repeat_rollouts: 每次 rollout 重复采样的对话数
# - custom_reward_function: 指定自定义奖励函数路径和名称
# - interaction_config_path: 用户模拟器交互配置文件路径
#
# 使用方式：sh recipe/collabllm/train_rl_collabllm.sh <可选的恢复训练路径>
