...
该脚本将 HuggingFace 上的 CollabLLM 多轮对话数据集转换为 verl 框架所需的格式：
- RL 模式（dataset_type=rl）：将原始数据转换为 (prompt, ground_truth, extra_info) 格式，
  用于强化学习训练，并去除重复提示。
- SFT 模式（dataset_type=sft）：将对话数据整理为完整对话序列，用于监督微调。

支持的数据集包括：math-hard-large、medium-large、bigcodebench-large 等。
处理后的数据以 parquet 格式保存，分为训练集和验证集。
"""
