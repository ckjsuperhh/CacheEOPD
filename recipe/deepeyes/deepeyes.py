...
"""
DeepEyes Recipe 核心模块 —— 多模态多轮视觉工具调用的强化学习训练。

本模块实现了 DeepEyes 论文（https://arxiv.org/abs/2505.14362）在 verl 框架上的复现，
支持视觉语言模型通过强化学习（GRPO）学会"用图像思考"。

主要组件：
  1. CustomRLHFDataset: 自定义数据集类，支持多模态输入和图像缩放工具（image_zoom_in_tool）
  2. compute_score: 奖励评分函数，结合 LLM-as-Judge 语义评估、格式奖励和工具使用奖励

该模块通过 LLM-as-Judge（如 Qwen2.5-72B-Instruct）对模型输出进行语义评估，
同时鼓励模型正确使用图像缩放工具来辅助视觉推理。
"""
import io
import logging
import os
...
# 初始化 LLM-as-Judge 客户端（用于奖励评分的语义评估）
...
class CustomRLHFDataset(RLHFDataset):
    """
    DeepEyes 自定义 RLHF 数据集类。

    继承自 verl 框架的 RLHFDataset，为多模态多轮视觉工具调用场景做了定制：
    - 设置系统提示词（system prompt）引导模型使用工具辅助推理
    - 支持图像缩放工具（image_zoom_in_tool）的参数配置
    - 处理多模态输入（图像 + 文本）的 tokenization 和 position_ids 计算
    """
    def __getitem__(self, item):
        """
        获取单个数据样本。

        构建包含系统提示和用户查询的消息列表，处理多模态输入（图像），
        并进行 tokenization、position_ids 计算等预处理。
        同时返回 raw_input_ids 以便与其他 chat template 组合使用。

        参数:
            item: 数据索引

        返回:
            包含 input_ids、attention_mask、position_ids、tools_kwargs 等的字典
        """
...
def compute_score(data_source: str, solution_str: str, ground_truth: str, extra_info=None) -> float:
    ...
    计算模型输出的奖励分数，支持多种格式的鲁棒处理。

    评分由三部分组成：
    - 准确性奖励（权重 0.8）：通过 LLM-as-Judge 判断答案是否语义正确
    - 格式奖励（权重 0.2）：检查输出是否遵循 <think>/<answer> 等标签格式
    - 工具奖励（权重 1.2）：当答案正确且使用了图像缩放工具时给予额外奖励

    参数:
        data_source: 数据来源标识
        solution_str: 模型生成的完整解答文本
        ground_truth: 标准答案
        extra_info: 额外信息字典，包含 question 等字段
