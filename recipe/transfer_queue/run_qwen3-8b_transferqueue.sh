# ============================================================
# 脚本用途：使用 TransferQueue 启动 Qwen3-8B 模型的 GRPO 训练
# ============================================================
# 启动的训练任务：
#   - 基于 verl 的 PPO/GRPO 强化学习训练
#   - 使用 TransferQueue 进行异步数据传递
#   - 异步 rollout 模式（vllm 后端）
# 关键参数含义：
#   - algorithm.adv_estimator=grpo: 使用 GRPO 优势估计器
#   - data.train_batch_size=128: 训练批次大小
#   - data.max_prompt_length=2048: 最大 prompt 长度
#   - data.max_response_length=8192: 最大响应长度
#   - actor_rollout_ref.rollout.mode=async: 异步 rollout 模式
#   - actor_rollout_ref.rollout.n=5: 每个 prompt 采样 5 个响应
#   - TQ_ZERO_COPY_SERIALIZATION: TransferQueue 零拷贝序列化开关
# ============================================================
set -x
