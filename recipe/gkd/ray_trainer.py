...
"""
GKD 在线策略蒸馏训练器模块（基于 Ray 单控制器架构）。

该模块实现了 GKD 知识蒸馏的分布式训练器 OnPolicyDistillTrainer，核心功能包括：
- GenerationBatchFuture: 封装异步生成结果的未来对象
- OnPolicyDistillTrainer: 继承自 RayPPOTrainer 的蒸馏训练器，支持：
  - 教师模型知识获取（通过 ZMQ 客户端）
  - one_step_off / two_step_off 两种异步调度策略，重叠 rollout/teacher/training 阶段
  - Actor-Rollout 分离架构下的权重同步管理
"""
...
class GenerationBatchFuture:
    ...
    该类将 Rollout 生成的序列结果（gen_batch_output）和后续获取的教师知识
    （teacher_batch_output）绑定在一起，支持异步流水线调度。
    """
    ...
    def __init__(self, epoch, batch, gen_batch_output):
        ...
        :param epoch: 当前训练轮次
        :param batch: 输入批次数据
        :param gen_batch_output: 主模型生成的序列（DataProtoFuture）
        """
        self.epoch = epoch
        self.batch = batch
    ...
    def set_teacher_batch_output(self, teacher_batch_output):
        ...
        将教师模型的蒸馏知识（DataProtoFuture 或原始输出）绑定到
        此生成批次对象，后续训练时使用。
        """
    ...
    def get(self):
        ...
        调用 gen_batch_output 和 teacher_batch_output 的 get() 方法
        以阻塞等待异步结果完成。

        Returns:
            tuple: (epoch, batch, gen_batch_result, teacher_batch_result)
                如果无教师输出，则返回 (epoch, batch, gen_batch_result)
        """
...
class OnPolicyDistillTrainer(RayPPOTrainer):
    ...
    继承自 RayPPOTrainer，专为知识蒸馏定制，主要扩展：
    - 集成 TeacherClient 获取教师模型 top-k 分布
    - 实现 one_step_off / two_step_off 两种异步调度策略
    - 管理 Actor-Rollout 分离架构（独立的 Actor Worker 和 Rollout Worker）
    - 支持异步序列生成与教师知识获取的流水线重叠
