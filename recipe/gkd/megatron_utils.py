...
"""
Megatron 权重转换工具模块（GKD 定制版）。

该模块为 GKD recipe 提供 Megatron 模型权重到推理引擎（vLLM/SGLang）格式的转换功能。
主要功能：
- per_tensor_generator: 逐层遍历 Megatron 模型参数，执行跨 PP/TP/EP 的权重收集与格式转换
- 支持 MoE（混合专家）模型的权重聚合
- 支持张量并行参数的 all-gather 和名称规范化

与 verl 原生 megatron_utils 的区别：本版本针对 Actor-only + Rollout-only 的
分离架构进行了优化，权重仅需 gather 到 rank 0 再广播给 rollout，而非全量 allgather。
"""
...
def per_tensor_generator(
    ...
    """逐层生成 Megatron 模型参数并转换为推理引擎格式。

    该生成器函数遍历 Actor 模型的所有参数，处理跨 PP（流水线并行）、
    TP（张量并行）和 EP（专家并行）的权重收集与格式转换。

    工作流程：
    1. 收集所有 PP rank 的参数元信息（名称、形状、dtype）
    2. 通过 PP 通信将参数从各 rank 汇聚到 rank 0
    3. 对 MoE 专家参数执行 EP gather
    4. 对 TP 分片参数执行 all-gather 合并
    5. 调用 weight_converter 完成名称和格式转换

    Args:
        actor_module: Megatron Actor 模型模块列表（支持 VPP）
        model_config: 模型配置（如 tie_word_embeddings 等）
        weight_converter: 权重转换器，负责名称映射和参数格式转换
        transformer_config: Megatron TransformerConfig 配置
        layer_name_mapping: 层名称映射（如 qkv、gate_up 等）
        convert_qkv_gate_up_by_simple_split: 是否使用简单切分方式转换 QKV/Gate-Up 权重

    Yields:
        (converted_name, converted_param): 转换后的参数名称和张量
    """
