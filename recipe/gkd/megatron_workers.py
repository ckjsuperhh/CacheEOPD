...
"""
GKD 知识蒸馏的 Megatron Worker 模块。

该模块定义了 GKD recipe 中基于 Megatron 后端的两个核心 Worker：
1. MegatronOnPolicyDistillActorWorker: Actor-only Worker，拥有可训练的 Megatron 模型
   和优化器，负责执行 KL 散度前向/反向传播和参数更新
2. MegatronOnPolicyDistillRolloutWorker: Rollout-only Worker，拥有推理引擎
   （vLLM/SGLang），负责生成序列（rollout）

此外还包含：
- TensorBuffer: 权重同步时的批量传输缓冲区
- OnPolicyDistillActor: Actor 训练逻辑的核心实现（前向/反向 + 优化器步进）
- record_time: 计时装饰器
"""
...
class TensorBuffer:
    ...
    将多个小的参数张量打包到一个连续的大缓冲区中，减少 NCCL broadcast 的调用次数，
    从而提升 Actor 到 Rollout 的权重同步效率。

    用法：先 append 多个张量，当缓冲区满时调用 broadcast 一次性发送，
    然后 clear 重置缓冲区继续填充。
    """
...
class OnPolicyDistillActor:
    ...
    该类的核心职责是执行 KL 散度的前向/反向传播和优化器更新步骤：
    - forward_backward_batch: 构造微批次，使用 Megatron 流水线并行执行前向/反向
    - update_policy: 协调清零梯度、前向/反向、优化器步进等完整训练步骤
    - 在流水线最后一个 stage 注入教师 top-k 分布信息，通过 logits_processor 计算 KL 损失
    """
...
class MegatronOnPolicyDistillActorWorker(ActorRolloutRefWorker):
    ...
    Actor-only Worker：拥有可训练的 Megatron 模型和优化器。

    该 Worker 仅承担 Actor 角色（不兼做 Rollout），负责：
    - 构建 Megatron 模型和分布式优化器
    - 执行知识蒸馏的训练步骤（update_actor）
    - 将训练后的权重同步给 Rollout Worker（sync_rollout_weights）
    - 管理检查点保存和加载
    """
...
class MegatronOnPolicyDistillRolloutWorker(ActorRolloutRefWorker):
    ...
    Rollout-only Worker：拥有推理引擎并负责序列生成。

    该 Worker 仅承担 Rollout 角色（不兼做 Actor），负责：
    - 构建 vLLM/SGLang 推理引擎
    - 同步/异步生成序列（generate_sequences / async_generate_sequences）
    - 从 Actor Worker 接收权重更新（sync_rollout_weights），支持批量桶式传输
    """
