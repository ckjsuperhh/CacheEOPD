...
提供从教师模型获取 top-k 预测和对数概率的工具函数，
用于 GKD 在线策略蒸馏训练。支持同步和异步两种获取方式。

核心函数：
    get_teacher_knowledge: 从教师服务端获取 top-k 预测的 logps 和 token 索引
"""
...
# 全局预分配的 padding 缓冲区，避免每次调用重复分配内存
...
def get_teacher_knowledge(batch: DataProto, teacher_client, n_server_workers=1, is_async=False):
    ...
    从教师模型获取 top-k 预测和对数概率，用于知识蒸馏。

    该函数执行以下操作：
    1. 从 batch 中提取有效 token（通过 attention_mask 过滤）
    2. 将输入分片后通过 teacher_client 发送给教师服务端
    3. 接收教师返回的 top-k logps 和 token 索引
    4. 将结果 padding 回原始 batch 形状

    Args:
        batch (DataProto): 输入批次，包含 input_ids 和 attention_mask
        teacher_client: 教师模型 ZMQ 客户端
        n_server_workers (int): 教师服务端 worker 数量，用于分片请求
        is_async (bool): 是否使用异步处理

    Returns:
        若 is_async=True: 返回 SimpleNamespace 对象，其 get() 方法延迟处理 futures
        若 is_async=False: 直接返回处理后的 DataProto，包含教师知识
    """
