...
该模块封装了 vLLM LLM 引擎，用于教师模型提供 top-k logprobs 推理服务。
主要内容：
- 对 vLLM 的 LogprobsProcessor 进行 monkey-patch，使其返回原始张量而非字典
- LogprobsTensors: 轻量级 NamedTuple 封装 logprob 张量数据
- VLLMEngine: 封装 vLLM LLM 引擎的 get_topk_logprobs 方法
"""
...
class LogprobsTensors(NamedTuple):
    ...
    存储 logprob 相关的三个张量：token ids、logprobs 值和选中 token 的秩。
    提供 CPU 转移、切片和创建空张量的便捷方法。
    """
    # [num_reqs, max_num_logprobs + 1]
    logprob_token_ids: torch.Tensor  # logprob 对应的 token ID
    # [num_reqs, max_num_logprobs + 1]
    logprobs: torch.Tensor  # logprob 值
    # [num_reqs]
    selected_token_ranks: torch.Tensor  # 被选中 token 的秩
...
class VLLMEngine:
    ...
    封装 vLLM LLM 引擎，提供 top-k logprobs 推理能力。
    支持 prompt logprobs（输入部分的 logprobs）和生成部分的 logprobs。
    """
    def __init__(self, ckpt_path, n_logprobs=0, tp_size=1):
        ...
        Args:
            ckpt_path: 模型检查点路径
            n_logprobs: 返回的 top-k logprobs 数量
            tp_size: 张量并行大小
        """
