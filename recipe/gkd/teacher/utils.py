...
提供序列化/反序列化和列表分块等通用工具函数，
用于教师模型 worker 和客户端之间的数据传输。

函数：
    chunk_list: 将列表等分为 n 个子列表
    serialize: 使用 torch.save 将数据序列化为字节流
    deserialize: 使用 torch.load 将字节流反序列化为 Python 对象
"""
...
def serialize(data):
    """将数据序列化为字节流。使用 torch.save 进行序列化。"""
    buffer = io.BytesIO()
...
def deserialize(message):
    """将字节流反序列化为 Python 对象。使用 torch.load 进行反序列化。"""
