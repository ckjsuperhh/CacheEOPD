...
该模块实现了 TeacherClient 类，通过 ZeroMQ REQ 套接字与教师模型服务端通信。
支持：
- 异步提交请求并获取 Future 结果
- 多线程后台任务处理
- 多 worker 负载均衡（当 n_server_workers > 1 时）
- 可配置的微批次大小、温度参数、最大序列长度等
"""
...
def check_if_invalid(topk_logps, inputs):
    """检查教师返回的 top-k logps 是否包含无效值（NaN/Inf/零值）。
    调试用函数，发现异常时记录到 teacher_debug.log 文件。
    """
    is_valid = True
    reason = ""
    for x in topk_logps:
...
class TeacherClient:
    ...
    通过 ZeroMQ REQ 套接字与教师模型服务端建立连接，支持异步提交推理请求。
    内部使用后台线程处理请求的发送和结果接收，返回 concurrent.futures.Future 对象。

    使用流程：
    1. 创建 TeacherClient 实例（自动启动后台线程）
    2. 调用 submit() 提交 token ids，获得 Future
    3. 调用 future.result() 获取 (responses, topk_logps, topk_indices)
    """
    def __init__(
        self,
        server_ip,
