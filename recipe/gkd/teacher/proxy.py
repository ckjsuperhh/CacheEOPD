...
该脚本实现了一个 ROUTER-DEALER 模式的 ZMQ 代理（proxy），
用于在多个客户端（训练进程）和多个教师模型 worker 之间路由消息。
- 前端（ROUTER）：监听客户端连接，接收推理请求
- 后端（DEALER）：连接 worker，分发请求并收集结果

环境变量：
    PROXY_FRONTEND_PORT: 前端监听端口（客户端连接此端口）
    PROXY_BACKEND_PORT: 后端监听端口（worker 连接此端口）
"""
