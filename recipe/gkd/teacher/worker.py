...
"""
教师模型 ZMQ Worker 服务端模块。

该脚本作为教师模型推理服务的 worker 进程运行，通过 ZMQ REP 套接字
连接到代理（proxy）的后端端口，循环处理客户端请求。

工作流程：
1. 反序列化收到的请求（包含 prompt_token_ids）
2. 调用推理引擎（vLLM）计算 top-k logprobs
3. 序列化并返回结果

支持通过命令行参数配置推理引擎后端、TP 大小、logprobs 数量等。
"""
