<!--
GKD 异步在线策略知识蒸馏训练器 Recipe

本 recipe 实现了基于 KL 散度的在线策略知识蒸馏（GKD）训练流程，核心特点：
1. 学生模型从当前策略生成样本，教师模型提供 top-k logprobs 作为蒸馏目标
2. 使用稀疏 KL 散度损失（仅关注 top-k token）进行训练
3. 支持 one_step_off / two_step_off 两种异步调度策略，重叠 rollout/teacher/training 阶段以提升吞吐
4. Actor 和 Rollout 使用独立 GPU 资源池的分离架构
5. 基于 Megatron（训练）+ vLLM/SGLang（推理）后端

关键文件：
- main_gkd.py: 训练入口
- ray_trainer.py: 分布式训练器（含异步调度策略）
- megatron_workers.py: Megatron Actor/Rollout Worker
- megatron_kl_loss.py: 词汇并行 KL 散度损失
- teacher/: 教师模型服务端（ZMQ 架构）
-->
