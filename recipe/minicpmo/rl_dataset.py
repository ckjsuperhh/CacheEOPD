...
"""
MiniCPM-o 多模态强化学习数据集模块。

本模块为 MiniCPM-o（开放多模态大模型）提供 RL 训练数据集处理逻辑。
主要功能包括：
- 图像切片（slice_image）：将大图切分为多个 patch 以适配模型输入
- 数据预处理（preprocess）：将多模态对话转换为模型可接受的输入格式
- RLHFDataset：从 Parquet 文件加载 RLHF 数据，支持图像/视频处理、
  prompt 过滤、断点恢复等功能
在 EOPD/verl 框架中，本模块替代默认的 rl_dataset，
以适配 MiniCPM-o 的特殊输入格式需求。
"""
