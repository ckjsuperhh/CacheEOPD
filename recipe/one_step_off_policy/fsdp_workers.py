...
"""
One-Step-Off Policy 的 FSDP Worker 模块。

该模块定义了分离式架构下基于 FSDP（Fully Sharded Data Parallel）后端的 Worker：
- DetachActorWorker: 分离式 Actor Worker，仅负责策略训练和权重同步
- DetachAsyncRolloutWorker: 分离式异步 Rollout Worker，仅负责序列生成
- CriticWorker: Critic Worker，负责价值函数训练

与标准 verl Worker 的区别：
- Actor 和 Rollout 运行在不同的 GPU 资源池上
- 通过 Ray collective 或 NPU 专用通信进行权重同步
- Rollout Worker 支持异步生成模式
"""
