...
"""
One-Step-Off Policy 分布式 PPO 训练器模块。

该模块实现了 OneStepOffRayTrainer，继承自 RayPPOTrainer，核心创新：
1. Actor 和 Rollout 使用独立的 GPU 资源池（分离架构）
2. 使用异步 rollout 管理器（OneStepOffAgentLoopManager）实现一步超前的流水线训练
3. 在当前步骤训练 Actor 时，同时异步生成下一步的 rollout 数据
4. 支持 rollout correction（重要性采样校正）处理 off-policy 偏差
5. 支持 FSDP/FSDP2 和 Megatron 两种后端策略

训练流程关键步骤：
- _async_gen_next_batch: 异步生成下一步的 rollout 数据
- sync_rollout_weights: Actor 权重同步到 Rollout
- fit: 主训练循环，交替执行生成和训练
"""
...
class OneStepOffRayTrainer(RayPPOTrainer):
    """One-Step-Off Policy 分布式 PPO 训练器。

    继承自 RayPPOTrainer，核心扩展：
    - 分离式 Actor-Rollout 架构：Actor 和 Rollout 各占独立 GPU 资源池
    - 异步 rollout 管理：通过 OneStepOffAgentLoopManager 实现一步超前生成
    - 权重同步组：通过 Ray collective 或 NPU 专用方式创建 Actor-Rollout 通信组
    - Rollout correction：支持重要性采样校正和拒绝采样处理 off-policy 偏差
    - 异步奖励计算：支持在 rollout 完成时立即启动个体奖励计算
