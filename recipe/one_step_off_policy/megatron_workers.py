...
"""
One-Step-Off Policy 的 Megatron Worker 模块。

该模块定义了分离式架构下基于 Megatron 后端的 Worker：
- DetachActorWorker: 分离式 Actor Worker，使用 Megatron 进行策略训练
- DetachAsyncRolloutWorker: 分离式异步 Rollout Worker，使用 vLLM/SGLang 推理
- CriticWorker: Critic Worker，使用 Megatron 进行价值函数训练

与 FSDP 版本类似，但使用 Megatron 的流水线并行和张量并行支持大规模模型训练。
通过 vllm_stateless_init_process_group 建立 Actor-Rollout 的权重同步通信组。
"""
