<!--
DAPO（Decoupled Clip and Dynamic Sampling Policy Optimization）算法 Recipe 文档。

本目录包含 DAPO 算法在 verl 框架上的完整实现和复现脚本。
DAPO 通过四项关键改进提升大规模 LLM 强化学习训练效果：
  1. 分离裁剪比率（Clip-Higher）：对策略比率使用不同的上下界裁剪
  2. 动态采样（组过滤）：过滤掉所有输出奖励相同的 prompt，只保留有区分度的样本
  3. Token 级别损失聚合：对所有 token 取平均而非序列取平均
  4. 过长输出惩罚缓冲区：线性惩罚超过阈值的输出长度
在 Qwen2.5-32B 上训练后，AIME 2024 准确率可达 52%。
-->
