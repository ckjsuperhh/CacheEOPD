#!/usr/bin/env bash
# =============================================================================
# DAPO 训练启动脚本 - Qwen2.5-32B 带 Rollout 校正
#
# 用途：在 DAPO 基础上启用 Rollout 校正机制，修正训练-推理分布不匹配问题。
# 训练任务：DAPO + Rollout Correction，包含：
#   - 序列级截断重要性采样（TIS）: rollout_is=sequence, threshold=2.0
#   - 几何拒绝采样（RS）: rollout_rs=geometric, threshold=1.01/0.99
#   - Token 级 veto 机制: token_veto_threshold=1e-4
# 注意：当前 server mode（agent loop）暂不支持 Rollout Correction。
# 参考文献：https://richardli.xyz/rl-collapse
# =============================================================================
