...
"""
InfiGUI-G1 自适应探索奖励（AER）函数模块。

本模块实现了 GUI 接地任务的奖励计算逻辑，包括：
- 格式奖励：检查输出是否符合 <think>...</think> 格式和 JSON 点列表格式
- 共线性检查：防止预测点在同一直线上
- 精度奖励：基于零中心对称公式计算，奖励 = 1/sqrt(n*k)，
  其中 n 为预测点数，k 为第一个正确点的排名
- 主调度函数 aer_gui_reward_function：根据 data_source 分派到具体奖励函数
"""
