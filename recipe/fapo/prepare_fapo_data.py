...
将多个数学推理基准数据集（AIME 2024/2025、GPQA Diamond、DAPO-Math-17k）
转换为 verl 框架所需的 parquet 格式。训练数据使用 DAPO-Math-17k（重复 20 次），
测试数据由多个基准测试集组成（每个重复多次以匹配 batch size）。

每条数据包含：
- prompt: 数学问题提示（附带 "Please reason step by step" 指令）
- reward_model: 规则奖励配置（ground_truth 答案）
- extra_info: 问题原文、数据集分割标记（train/test）
"""
