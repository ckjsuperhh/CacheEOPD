...
# FAPO 32B 基线模型训练脚本
#
# 用途：使用标准 GRPO 算法训练 32B 参数的数学推理模型（不含 FAPO 的 flawed positive 检测）。
# 启动的训练任务：通过 Ray 提交到集群的 verl PPO/GRPO 训练。
# 关键参数：max_prompt_length=2K, max_response_length=20K, 8 节点 64 GPU 训练。
