#!/usr/bin/env bash
# FAPO 32B 模型训练脚本（远程 GenRM 服务模式）
#
# 用途：使用 FAPO 算法训练 32B 参数的数学推理模型，
# 通过外部远程 GenRM 服务进行 flawed positive 检测。
# 注意：需提前启动外部 GenRM 服务并在 reward_fn_reasoning_remote.py 中配置地址。
