<!-- 
  recipe/ 目录是 EOPD/verl 框架的"配方"集合。
  每个子目录代表一种特定的 RL 训练方案或扩展功能，
  通过将 verl 作为库导入，并在此基础上添加特定的训练流程扩展。
  包含的 recipe 涵盖：异步离策略训练（fully_async_policy）、
  高性能流式数据管理（transfer_queue）、VLA 模型强化学习（vla）、
  推测解码加速（specRL）、数学推理（open_math_reasoning）、
  GUI 接地（infigui-g1）等多种研究方向。
-->
# Recipe
