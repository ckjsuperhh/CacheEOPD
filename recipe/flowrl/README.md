<!--
  FlowRL Recipe 说明文档：
  FlowRL 通过流平衡（Flow Balance）匹配奖励分布，实现大语言模型的强化学习训练。
  包含模型准备、数据准备、训练配置和评估等完整流程。
-->
<!--
中文摘要：
FlowRL 是一种基于流平衡（Flow Balance）的强化学习推理优化方法。
其核心思想是通过匹配奖励分布来改进策略优化，替代传统的优势函数估计方法。
该 recipe 提供了 FlowRL 在 verl 框架中的完整实现，包括：
1. FlowRLActor：FlowRL 专用的策略 Actor
2. FlowRL FSDP Worker：分布式训练工作节点
3. RayFlowRLTrainer：基于 Ray 的分布式训练器
4. 数据和模型准备脚本（prepare/）
-->
