...
中文说明：
本模块是 SpecRL 版 PPO 训练的主入口。
SpecRL 通过推测解码（Speculative Decoding）加速 RL rollout 生成，
利用历史响应作为 draft 序列，在数学和代码等结构化任务中
可获得最高 2.1x 的加速。主要组件包括：
- SpecRLTaskRunner: 管理训练流程的任务运行器
- SpecRLRayPPOTrainer: 集成了后缀缓存管理的 PPO 训练器
"""
