...
中文说明：
本模块定义了 VLA 专用的 FSDP 工作节点（RobActorRolloutRefWorker）。
在标准 verl ActorRolloutRefWorker 基础上增加了：
- VLA 模型（OpenVLA）的特殊初始化（注册自定义 HuggingFace 配置）
- 自定义 rollout 构建（NaiveRolloutRob，直接调用模型前向传播生成动作）
- 训练/推理模式切换（rollout_mode / trainer_mode）
- 支持 FSDP 状态字典管理
"""
