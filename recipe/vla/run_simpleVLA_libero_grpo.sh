# ============================================================
# 脚本用途：在共置模式下训练 SimpleVLA-OFT 模型（Libero/IsaacSim）
# ============================================================
# 启动的训练任务：VLA 模型 REINFORCE++ 训练
# 关键参数：
#   - env.disagg_sim.enable=False（默认共置模式）
#   - trainer.n_env_gpus_per_node: 仿真使用的 GPU 数
#   - trainer.n_rollout_gpus_per_node: 训练/推理使用的 GPU 数
#   - 其余参数同 run_simpleVLA_isaac_disagg.sh
# 特殊处理：在 Hopper GPU 上自动启用 MUJOCO_GL=osmesa
# ============================================================
set -x
