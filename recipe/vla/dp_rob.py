...
中文说明：
本模块实现了 VLA 模型的数据并行 PPO Actor（RobDataParallelPPOActor）。
与标准 PPO Actor 的主要区别在于：
- 处理 VLA 模型的特殊输出格式（从 vocab_size-256-64 到 vocab_size-64 的 256 维 logits）
- 支持轨迹级 masking（根据任务完成状态生成 response_mask）
- 支持 FSDP 和动态批次大小
- 动作空间为 256 个离散 token（对应机器人控制动作）
"""
