# ============================================================
# 脚本用途：在解聚模式下使用 IsaacSim 训练 SimpleVLA-OFT 模型
# ============================================================
# 启动的训练任务：VLA 模型 GRPO/REINFORCE++ 训练
# 关键参数：
#   - env.disagg_sim.enable=True: 启用解聚模式（训练和仿真在不同节点）
#   - SIM_TYPE: 仿真器类型（isaac 或 libero）
#   - STAGE_NUM: 流水线阶段数
#   - BATCH_SIZE: 训练批次大小
#   - ROLLOUT_N: 每个 prompt 的采样数（需等于 num_envs）
#   - MAX_EPISODE_STEPS: 最大回合步数（libero 基准要求 512）
#   - algorithm.adv_estimator=reinforce_plus_plus: 优势估计器
# 前置条件：需先建立 Ray 集群连接（训练节点 + 仿真节点）
# ============================================================
#!/bin/bash
set -x
