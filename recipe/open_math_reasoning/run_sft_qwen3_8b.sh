# ============================================================
# 脚本用途：在 OpenMathReasoning 数据集上对 Qwen3-8B 进行 SFT 训练
# ============================================================
# 启动的训练任务：使用 torchrun 启动分布式 SFT 训练
# 关键参数：
#   - BACKEND: 训练后端（fsdp/fsdp2/megatron）
#   - MODEL_ID: 预训练模型 ID
#   - TRAIN_FILES: 训练数据路径（CoT 数据集 parquet）
#   - SP_SIZE / FSDP_SIZE / TP_SIZE / PP_SIZE: 各种并行度配置
#   - optim.lr=2e-5: 学习率
#   - data.max_length=32768: 最大序列长度
#   - PAD_MODE: 填充模式（no_padding 等）
# ============================================================
#!/usr/bin/env bash
set -xeuo pipefail
