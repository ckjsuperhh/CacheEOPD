...
run_root=/dev/shm/cacheeopd/cacheeopd_three_methods_20260804
log_root=/home/kejiechen/CacheEOPD/logs/cacheeopd_three_methods_20260804
...
evaluate_checkpoints() {
    ...
    for step in 50 100 150 200 250 300; do
        ...
        final_path="$log_root/${tag}_student_step${step}.jsonl"
...
run_one() {
    ...
    case "$method" in
        fused)
            mode_args=(--mode fused)
            ;;
        mixed)
            mode_args=(--mode mixed --fused-prob 0.5)
            ;;
        anneal)
            mode_args=(--mode anneal --anneal-start-prob 1.0 --anneal-end-prob 0.0 \
    ...
    "$python_bin" -m cache_eopd.train_student_distill \
        ...
        --teacher-device "cuda:$gpu" --teacher-gpus "$gpu" "${mode_args[@]}" \
