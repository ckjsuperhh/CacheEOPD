...
"""
基于 Ray 的单控制器（single-controller）分布式执行层。

【verl 框架】本文件是 verl「hybrid-controller」架构的核心：
- 训练主循环（driver / controller，即 RayPPOTrainer）在单进程跑控制流；
- 真正吃 GPU 的算子（actor 更新、rollout 生成、ref 前向）被分发到一组 Ray Actor（worker）上并行执行。
关键抽象：
- `RayResourcePool`：一组 GPU 资源池，决定 worker 如何调度/放置（bin_pack、colocate）。
- `RayClassWithInitArgs`：把一个「类 + 构造参数」打包，便于延迟实例化到不同资源池。
- `RayWorkerGroup`：一组等价 worker 的集体句柄。通过 `execute_*` 方法把调用广播/分发到各 worker：
  - `execute_all`：每个 worker 都跑（如 update_actor）；
  - `execute_rank_zero`：只在 rank-0 worker 跑（如保存）；
  - 配合 `@register(dispatch_mode=...)` 装饰器，决定输入 DataProto 如何按 batch 维切分到各 worker。
- 与 `verl/single_controller/base/` 里的 Worker / WorkerGroup 抽象解耦，使单机（无 Ray）也能跑。

EOPD 视角：ray_trainer 里的 `self.actor_rollout_wg`、`self.ref_policy_wg` 都是 RayWorkerGroup；
