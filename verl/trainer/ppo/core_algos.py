...
本文件实现了各种 PPO 类算法的核心计算逻辑，包括：
- 优势函数估计（GAE、GRPO、RLOO、REINFORCE++、ReMax、GPG、OPO 等）
- 策略损失函数（vanilla PPO、on-policy distillation、GSPO、SAPO、GPG、Clip-Cov、KL-Cov、CISPO 等）
- 熵损失、价值函数损失
- KL 散度惩罚计算
- 损失聚合（agg_loss）

【EOPD 核心】本文件包含 `compute_policy_loss_on_policy_distill` 函数，
这是 EOPD (Entropy-gated On-Policy Distillation) 的核心损失实现：
- 使用 clipped reverse-KL 作为策略梯度优势 (teacher_log_prob - student_log_prob)
- 在教师分布高熵 token 上额外施加 forward-KL（Soft KD）蒸馏损失
- 通过 entropy gating 机制选择性地在教师不确定的 token 上施加额外监督

与其他模块的关系：
- 被 `dp_actor.py` 中的 `update_policy()` 调用以计算策略梯度损失
- 被 `ray_trainer.py` 中的训练循环调用以计算优势函数
- 依赖 `verl.utils.torch_functional` 中的 masking/whitening 工具函数
"""
...
def compute_policy_loss_on_policy_distill(
    ...
    # 【EOPD 核心】PPO 裁剪比率：ratio = π_current / π_old_actor
    # old_log_prob 是学生模型更新前的 log_prob（用于 PPO 重要性采样比率）
    old_log_prob = old_log_prob.detach()
    ratio = torch.exp(log_prob - old_log_prob)  # ratio = exp(log π_θ - log π_θ_old) = π_θ / π_θ_old
    ...
    # 【EOPD 核心】计算基于教师模型的优势函数
    # teacher_advantage = log π_teacher - log π_student_old（逐 token 的密集信号）
    # 这等价于 reverse KL 的逐 token 分解项：最小化 KL(π_student || π_teacher) 的梯度方向
    # 使用 detach() 阻断梯度，使优势函数仅作为常数系数参与策略梯度计算
    # Shape: (batch_size, response_length) - 每个 token 都有独立的优势值
    ...
    # 【EOPD 核心】Soft KD 损失：基于 entropy gating 的前向 KL 蒸馏
    # 仅在教师分布高熵（不确定性高）的 token 上施加 forward-KL 损失
    # 前向 KL = sum(teacher_probs * (log teacher_probs - log student_probs))
    # 这有助于学生在教师不确定的位置学习更平滑的分布
    soft_kd_loss = 0.0
    if teacher_topk_log_probs is not None and student_topk_log_probs is not None:
        ...
        # 【EOPD 核心】逐 token 前向 KL 散度（在 top-k 词汇子集上计算）
        # KL(teacher || student) = sum_k [ teacher_prob_k * (log teacher_prob_k - log student_prob_k) ]
        # 其中 k 遍历教师 top-k 词汇，这是 forward KL 的截断近似
        ...
        # 【EOPD 核心】Entropy Gating 机制：
        # soft_kd_entropy_mask: 控制哪些 token 参与 Soft KD 损失计算（教师熵 >= 阈值）
        # high_entropy_mask: 用于日志统计，记录高熵 token 的比例
        # 两个阈值可以不同：soft_kd_entropy_threshold 控制损失，high_entropy_threshold 控制监控
    ...
    # 【EOPD 核心】最终损失 = OPD 策略梯度损失 + Soft KD 蒸馏损失
    # OPD 部分：使用 clipped reverse-KL 优势的标准 PPO 损失
    # Soft KD 部分：entropy-gated forward-KL，仅在教师高熵 token 上计算
