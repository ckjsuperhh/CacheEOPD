...
"""
FSDP 后端下的 Worker 实现（actor / rollout / ref 三合一）。

本文件实现了 `ActorRolloutRefWorker`（及其异步变体 `AsyncActorRolloutRefWorker`）：
- 一个 worker 进程内可同时承载 actor（学生，可训练）、rollout（vLLM/SGLang 生成）、ref（冻结参考策略）三种角色
- 通过 FSDP 做全分片数据并行，支持 CPU offload、ulysses 序列并行、LoRA 等
- 提供 `update_actor`（训练）、`compute_ref_log_prob`（参考/教师前向）、`generate_sequences`（rollout）等核心方法

【EOPD 核心】在 EOPD 中，本文件的 `ref` 角色就是「教师模型」：
- ray_trainer 中若配置了 `teacher_model.path`，会把 RefPolicy worker 复用为教师（权重设为教师路径、去掉 optimizer）
- `compute_ref_log_prob()` 即教师前向：除返回 `ref_log_prob` 外，还会产出 EOPD 必需的
  `ref_entropy`（逐 token 熵，用于熵门控）、`ref_topk_log_probs` / `ref_topk_indices`（教师 top-k 分布，用于 forward-KL）
- 这些张量被 `core_algos.compute_policy_loss_on_policy_distill` 消费，构成 entropy-gated forward-KL 蒸馏损失
与 `dp_actor.py` 的关系：本文件负责「前向/更新」的底层执行，`update_actor` → `actor.update_policy()`
（在 dp_actor.py 中）才真正调用 EOPD 策略损失函数。
"""
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        if rollout_name in ("hf", "c2c_hf"):  # 【C2C 集成点】c2c_hf 与 hf 同为同步单进程 rollout
        ...
        if rollout_name == "c2c_hf":
            # 【C2C 集成点】路线 A：HF 后端 + teacher KV 融合。与 vllm/sglang 不同，
            # 它直接复用训练侧的 actor module（可传 past_key_values），不走 async server 注册表。
            from cache_eopd.c2c_hf_rollout import C2CHFRollout

            self.rollout = C2CHFRollout(module=self.actor_module_fsdp, config=self.config.rollout)
        else:
    ...
    def compute_ref_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            # 【EOPD 核心】教师前向（on-policy distillation 的蒸馏监督来源）
            # calculate_entropy=True：产出逐 token 熵，供 entropy gating 选择「教师不确定」的 token
            # topk=topk：要求返回教师 top-k logits 的 (log_probs, indices)，
            # 用于在 core_algos 中对学生做截断 forward-KL 蒸馏
            ...
            # 【EOPD 核心】把教师前向的三种信号打包回传，供 EOPD 策略损失使用：
            # - ref_log_prob：逐 token 教师 log_prob，用于 reverse-KL 优势 (teacher - student)
            # - ref_entropy：逐 token 教师熵，用于 entropy gating（仅在高熵 token 上施加 forward-KL）
            # - ref_topk_log_probs / ref_topk_indices：教师 top-k 分布，用于 forward-KL 的截断近似
            # - ref_full_log_probs：全序列 log prob（partial_solution 的 SFT 分支可能用到）
