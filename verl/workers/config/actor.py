...
"""
Actor / 策略损失相关配置（verl dataclass 配置）。

本文件定义了训练 actor（学生模型）所需的所有配置项，包括：
- `PolicyLossConfig`：策略损失配置（含 EOPD 的 soft_kd 相关开关）
- `ActorConfig` / `FSDPActorConfig` / `McoreActorConfig`：actor 训练总配置
- `RouterReplayConfig`：MoE 路由回放配置
...
class PolicyLossConfig(BaseConfig):
    ...
    Args:
        loss_mode (str): Loss function mode. Options: 'vanilla', 'clip-cov', 'kl-cov', 'gpg',
            'on_policy_distill'. 【EOPD 核心】设 'on_policy_distill' 即启用 EOPD：
            策略损失走 core_algos.compute_policy_loss_on_policy_distill，由 clipped reverse-KL
            （OPD 部分）+ entropy-gated forward-KL（Soft KD 部分）组成。
    ...
    # - True：学生在全词表上做 log_softmax（不重归一化），forward-KL 覆盖更完整但需全词表 logits。
    # 该开关在 dp_actor.py 的 update_policy 中通过 getattr(config.policy_loss, ...) 读取。
