...
"""
verl 的通用数据传输协议（DataProto）——贯穿整个训练/推理流水线的「数据容器」。

本文件定义了 verl 中函数与模块之间交换数据的标准协议：
- `DataProto`：核心数据结构，由三部分组成
  - `batch`：一个 `TensorDict`（来自 tensordict 库），把「一批同形张量」当作单个张量来操作；
    理想情况下所有共享同一 batch 维的张量（log_prob、advantages、attention_mask 等）都放进 batch
  - `non_tensor_batch`：非张量字段（如 uid、prompt 文本、reward 字符串），用普通 dict 存
  - `meta_info`：元信息（如 loss_mode、adv_estimator、temperature），不参与张量运算
- `DataProtoItem` / `DataProtoFuture`：单样本视图与异步结果封装
- 一系列工具函数：pad/unpad、chunk/split、fold/unfold、union、serialize/deserialize、all_gather 等，
  支撑分布式场景下的数据切分、拼接与跨进程传输

【verl 框架】为什么重要：EOPD 训练主循环里，rollout、ref 前向、advantage 计算、actor 更新
每一步的输入输出都是 DataProto。理解 `batch` / `non_tensor_batch` / `meta_info` 的分工，
是读懂 ray_trainer / dp_actor / fsdp_workers 的前提。例如 ray_trainer 把 teacher 的
ref_topk_log_probs、ref_entropy 写进 `batch`，再把 `loss_mode` 写进 `meta_info`。
"""
...
class DataProto:
    ...
    【verl 框架】核心容器速查：
    - `batch`（TensorDict）：所有同 batch 维的张量走这里，支持像张量一样整体 .to(device)、chunk、cat
    - `non_tensor_batch`（dict）：uid、原始文本等异构数据
    - `meta_info`（dict）：控制开关，如 EOPD 的 loss_mode / adv_estimator
    常用方法：from_dict（构造）、select/pop（取字段）、chunk/split（切 minibatch）、
    union（合并两个 DataProto 的字段）、make_iterator（PPO epoch 内迭代）、repeat（GRPO 同 prompt 多采样）。
    """
